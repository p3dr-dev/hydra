...
class RiskManager:
    ...
    def __init__(self, api_client: ApiClient, data_analyzer: DataAnalyzer):
        ...
        self.data_analyzer: DataAnalyzer = data_analyzer
        ...
        self.trading_parameters: dict[str, Any] | None = None
        self.risk_free_rate: Decimal = Decimal("0.02")  # 2% ao ano (pode ser ajustado dinamicamente)
        self.max_correlation_threshold: Decimal = Decimal("0.7")
        self.min_sharpe_ratio: Decimal = Decimal("0.5")

        # Configurações de risco - serão obtidas dinamicamente
        self.max_portfolio_risk: Decimal = Decimal("0.05")  # 5% máximo de risco por operação
        self.max_daily_loss: Decimal = Decimal("0.02")  # 2% máximo de perda diária
        self.position_sizing_method: str = "kelly"  # 'kelly', 'fixed', 'volatility'
        self.stop_loss_percentage: Decimal = Decimal("0.01")  # 1% stop-loss
        self.take_profit_percentage: Decimal = Decimal("0.02")  # 2% take-profit
        self.max_concurrent_positions: int = 5
        self.min_position_size: Decimal = Decimal(10)  # Valor mínimo em USDT
    ...
    def get_current_regime(self) -> str:
        """Retorna o regime operacional atual do RiskManager."""
        return getattr(self, "current_regime", "Balanceado")
    ...
    def get_dynamic_strategy_parameters(self, market_metrics: dict) -> dict:
        """
        Obtém os parâmetros de estratégia dinâmicos com base no regime operacional atual.
        """
        active_params = self.regime_parameters[self.current_regime]
        
        # A lógica de usar market_metrics para ajustar dinamicamente os parâmetros do regime pode ser adicionada aqui.
        # Por enquanto, estamos usando os valores definidos para o regime.
        
        min_profit_threshold = self.api_client.get_trading_fees().get('taker', 0.001) * active_params["profit_multiplier"]
    ...
    def _fetch_exchange_info_if_needed(self):
        ...
        if not self.exchange_info:
            ...
            self.exchange_info: dict[str, Any] | None = self.api_client.get_exchange_info()
            if not self.exchange_info:
    ...
    def get_balance(self, asset: str) -> Decimal:
        ...
        """
        try:
            account_info: dict[str, Any] = self.api_client.get_account_info()
    ...
    def _calculate_path_risk_score(self, path: list[str], tickers: dict[str, Any]) -> Decimal:
    ...
    def _estimate_spread(self, symbol: str, tickers: dict[str, Any]) -> Decimal:
    ...
    def _estimate_path_volatility(self, path: list[str], tickers: dict[str, Any]) -> Decimal:
    ...
    def _estimate_max_drawdown(self, path: list[str], tickers: dict[str, Any]) -> Decimal:
    ...
    def _calculate_correlation_score(self, path: list[str], tickers: dict[str, Any]) -> Decimal:
    ...
    def check_stop_loss_take_profit(self, current_prices: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def _get_current_price(self, asset: str, current_prices: dict[str, Any]) -> Decimal | None:
